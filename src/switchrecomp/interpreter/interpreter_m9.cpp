        ...
        for (const auto& instruction : block->instructions)
        {
            ...
            if (is_m9_opcode(instruction.opcode))
            {
                const auto executed = execute_m9_instruction(
                    instruction, runtime, read_value,
                    [&](std::uint64_t value, InterpreterValueProvenance) {
                        return store(value);
                    });
                if (!executed)
                {
            ...
            case ir::Opcode::GuestLoad:
            {
                ...
                const auto rc = runtime::switchrecomp_runtime_guest_load(
                    &runtime, address.value(), instruction.memory_size, &loaded);
            ...
            case ir::Opcode::GuestStore:
            {
                ...
                const auto rc = runtime::switchrecomp_runtime_guest_store(
