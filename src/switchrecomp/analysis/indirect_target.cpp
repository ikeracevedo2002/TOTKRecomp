    ...
    const ProcessImage* process_image, const IndirectTargetDiscoveryOptions& options)
{
    ...
    normalize_evidence(assessment.static_evidence);
    if (options.require_independent_static_evidence && assessment.static_evidence.empty())
    {
    ...
    if (!options.allow_runtime_cfg_promotion && assessment.static_evidence.empty())
    {
        decision.kind = IndirectTargetDecisionKind::InsufficientEvidence;
    ...
    decision.reason = graph.value().unresolved.empty()
