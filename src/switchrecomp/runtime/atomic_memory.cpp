...
[[nodiscard]] Result<void> validate_width(std::uint8_t size)
{
    if (size != 1U && size != 2U && size != 4U && size != 8U)
    {
        return Result<void>::failure(
            make_error(ErrorCode::InvalidAtomicWidth,
...
[[nodiscard]] Result<void> validate_alignment(std::uint64_t address, std::uint8_t size)
{
    ...
    {
        return Result<void>::failure(
            make_error(ErrorCode::MisalignedAtomicAccess,
...
{
    const auto found = shared.reservation_generations.find(granule);
    return found == shared.reservation_generations.end() ? 0U : found->second;
}
                              ...
                              std::uint8_t size) noexcept
{
    ...
    const auto last = granule_for(address + static_cast<std::uint64_t>(size) - 1U);
                                           ...
                                           std::uint8_t size)
{
    ...
    {
        return Result<void>::failure(
            make_error(ErrorCode::InvalidRuntimeContext,
                       "shared runtime state has no guest memory"));
    }
    const auto width = validate_width(size);
    if (!width)
    {
        return width;
    }
    const auto alignment = validate_alignment(address, size);
    if (!alignment)
    {
        return alignment;
    }
                                                ...
                                                std::uint8_t size)
{
    ...
    return Result<std::uint64_t>::success(
        load_little_endian(std::span<const std::byte>(bytes).first(size)));
}
                                                        ...
                                                        std::uint8_t size)
{
    ...
    if (!permissions)
    {
        return Result<void>::failure(permissions.error());
    }
                                        ...
                                        std::uint8_t size, std::uint64_t value)
{
    ...
    const auto write =
        shared.memory->write(address, std::span<const std::byte>(bytes).first(size));
    if (!write)
    {
        return write;
    }
...
void acquire_fence(ir::MemoryOrder order) noexcept
{
    if (order == ir::MemoryOrder::Acquire ||
        order == ir::MemoryOrder::SequentiallyConsistent)
    {
        std::atomic_thread_fence(order == ir::MemoryOrder::SequentiallyConsistent
                                        ...
                                        std::uint8_t size, ir::MemoryOrder order)
{
    ...
    if (!common)
    {
        return Result<std::uint64_t>::failure(common.error());
    }
    if (!valid_load_order(order))
    {
        return Result<std::uint64_t>::failure(
            make_error(ErrorCode::InvalidMemoryOrder,
                                     ...
                                     ir::MemoryOrder order)
{
    ...
    reservation.granule = granule_for(address);
    reservation.generation = generation_for(shared, reservation.granule);
                                      ...
                                      std::uint64_t value, ir::MemoryOrder order)
{
    ...
    {
        reservation.clear();
        return Result<std::uint32_t>::failure(common.error());
    }
    if (!valid_store_order(order))
    {
        reservation.clear();
    ...
    {
        reservation.clear();
        return Result<std::uint32_t>::failure(target.error());
    }

    const auto expected_granule = granule_for(address);
    const bool succeeds = reservation.valid && reservation.address == address &&
                          reservation.size == size && reservation.granule == expected_granule &&
                          reservation.generation == generation_for(shared, expected_granule);
    reservation.clear();
    if (!succeeds)
    {
    ...
    if (!stored)
    {
        return Result<std::uint32_t>::failure(stored.error());
    }
...
Result<void> memory_barrier(ir::BarrierKind kind, ir::BarrierOption option)
{
    switch (option)
    {
    case ir::BarrierOption::Sy:
    case ir::BarrierOption::St:
    case ir::BarrierOption::Ld:
    case ir::BarrierOption::Ish:
    case ir::BarrierOption::Ishst:
    case ir::BarrierOption::Ishld:
    case ir::BarrierOption::Nsh:
    case ir::BarrierOption::Nshst:
    case ir::BarrierOption::Nshld:
    case ir::BarrierOption::Osh:
    case ir::BarrierOption::Oshst:
    case ir::BarrierOption::Oshld:
        break;
    default:
        return Result<void>::failure(
            make_error(ErrorCode::InvalidBarrier, "invalid AArch64 barrier option"));
    }

    switch (kind)
    {
    case ir::BarrierKind::Dmb:
    case ir::BarrierKind::Dsb:
    case ir::BarrierKind::Isb:
        std::atomic_thread_fence(std::memory_order_seq_cst);
        return Result<void>::success();
    }
    return Result<void>::failure(
        make_error(ErrorCode::InvalidBarrier, "invalid AArch64 barrier kind"));
}
