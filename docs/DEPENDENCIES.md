...
| LLVM | 18.1.3 (`llvm-toolchain-18` Ubuntu Noble package `1:18.1.3-1ubuntu1`) | Semantic IR lowering, verification, and native ORC JIT | Apache-2.0 with LLVM exceptions | Optional `switchrecomp-codegen-llvm` backend | Host-provided `find_package(LLVM 18.1.3 CONFIG)`; not fetched by CMake |
...
API for raw blocks and does not add zstd for unsupported ZBIC data. LLVM is
optional and is used only by the `switchrecomp-codegen-llvm` backend; the IR,
lifter, verifier, and interpreter do not include LLVM headers. Remill, Unicorn,
QEMU, Vulkan, SDL, GLFW, and the Nintendo SDK are not dependencies. New dependencies must have a clear purpose, a pinned version or
